...
class MOT(DETECTION):
    def __init__(self, db_config, split=None, sys_config=None):
        ...
        super(MOT, self).__init__(db_config)
        ...
        if split is not None:
            ...
            if self._split == 'train':
                self._data_dir = os.path.join(coco_dir, 'train')
                self._anno_file = os.path.join(coco_dir, 'pseudo.json')
            else:
                self._data_dir = os.path.join(coco_dir, 'val')
                self._anno_file = os.path.join(coco_dir, 'gt.json')
